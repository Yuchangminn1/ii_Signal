...
public class TimeStampLogHandler : ILogHandler
{
    ...
    /// <param name="format">타임스탬프 포맷 (기본: "HH:mm:ss")</param>
        ...
        string format = "HH:mm:ss")
    {
        ...
        _format = string.IsNullOrEmpty(format) ? "HH:mm:ss" : format;
    }

    public void LogFormat(LogType logType, UnityEngine.Object context, string format, params object[] args)
    {
        var ts = _nowProvider().ToString(_format);
        // 원본 포맷 문자열 앞에 타임스탬프를 붙여서 넘깁니다.
        _defaultHandler.LogFormat(logType, context, $"[{ts}] {format}", args);
    }
